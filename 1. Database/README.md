...
**특징**
- EXPLAIN의 경우 쿼리 실행 계획을 세우고 실제로 쿼리 데이터를 읽지 않습니다.
- EXPLAIN ANALYZE의 경우 계획 수립 후 실제로 쿼리를 수행하여 actual time, row, buffers 통계를 포함합니다.
  - 추가 설명으로 만일, 5TB 이상의 non index, non partitioning table에 조회 쿼리를 잘 못 쓰게 되면 seq scan이 발생하며 1시간 이상의 트랜잭션 밀림이 발생 할 수 있습니다. 
  - 이런 현상을 막기 위해서는 기초적인 partitoning table 생성 및 자주 조회에 사용되는 컬럼에 인덱스를 걸어야 합니다.

### 9. MVCC와 Dead Tuples

**MVCC 개념**

- PostgreSQL에서 MVCC를 통해서 읽기 및 쓰기 충돌을 방지하고 각 쿼리의 스냅샷을 남깁니다. 만일, 기존 테이블에 인덱스 업데이트를하게 되면 MVCC가 그대로 기존 테이블의 스냅샷을 남기고 테이블에 인덱스를 그대로 붙이게 됩니다. (450MB 테이블이 인덱스를 걸고 나서 920MB이 됨)
  - 위와 같은 상황을 방지하기 위해서는 ALTER TABLE으로 INDEX를 붙이기 보다는 INDEX가 미리 지정된 신규 테이블을 생성하여 테이블 데이터를 옮기고 후에 기존 테이블을 삭제하는 식으로 지정해줘야 합니다.
- DeadTuples의 경우 각 행에 대해서 Update / Delete 발생 시 그대로 데이터가 남고 새로운 버전의 데이터가 삽입 될 때 사용 될 수 있습니다. 누적 될 시 디스크 풀이 찰 수 있습니다.  